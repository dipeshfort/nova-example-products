...
async function main() {
    ...
    const inserts = movies.map(({ id, title, overview, poster, releaseDateStr, rating }) => {
        return [id, title, overview, poster, releaseDateStr, rating]
    })
...
async function execute(inserts) {
    const text = `
        INSERT INTO ${TABLE_NAME} (id, title, overview, poster, release_date_str, rating)
        VALUES ($1, $2, $3, $4, $5, $6)
